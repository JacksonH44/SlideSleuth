    ...
    def _write_tiles(self):
        ...
        if self._xmlfile != '':
            ...
            if os.path.isfile(xmldir):
                ...
                if xml_valid == False:
                    print(
                        "Error: xml %s file cannot be read properly - please check format" % xmldir)
                    return
            else:
                ...
                for col in range(cols):
                    ...
                    if InsertBaseName:
                        ...
                        tilename_bw = os.path.join(
                            tiledir, '%d_%d_bw.%s' % (col, row, self._format))
                    if xml_valid:
                        ...
                        print(mask.shape)
                        print(mask[startIndX_current_level_conv[col]:endIndX_current_level_conv[col],
                                   startIndY_current_level_conv[row]:endIndY_current_level_conv[row]])
                        PercentMasked = mask[startIndY_current_level_conv[row]:endIndY_current_level_conv[row],
                                             startIndX_current_level_conv[col]:endIndX_current_level_conv[col]].mean()
                        if PercentMasked > 0:
    ...
    def _tile_done(self):
        ...
        if count % 100 == 0 or count == total:
            print("Tiling %s: wrote %d/%d tiles" %
                  (self._associated or 'slide', count, total), end='\r', file=sys.stderr)
            if count == total:
...
class DeepZoomStaticTiler(object):
    ...
    def _run_image(self, associated=None):
        ...
        tiler = DeepZoomImageTiler(dz, basename, self._format, associated, self._queue,
                                   self._slide, self._basenameJPG, self._xmlfile, self._mask_type)
        tiler.run()
    ...
    def _write_html(self):
        ...
        env = jinja2.Environment(
            loader=jinja2.PackageLoader(__name__), autoescape=True)
